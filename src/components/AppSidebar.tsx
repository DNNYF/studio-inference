...
import {
  ...
  SidebarSeparator,
  SidebarMenuBadge,
} from "@/components/ui/sidebar"; // Assuming this sidebar is part of shadcn
  ...
  return (
    <Sidebar collapsible="icon" className="border-r">
                   ...
                   <AlertDialog>
                      <AlertDialogTrigger asChild>
                         <Button
                           variant="ghost"
                           size="icon"
                           className="absolute right-1 top-1/2 -translate-y-1/2 h-6 w-6 p-0 opacity-0 group-hover/menu-item:opacity-100 group-data-[collapsible=icon]:hidden focus:opacity-100"
      ...
      <SidebarFooter className="p-2 space-y-1">
         <Button onClick={onExportAll} variant="ghost" className="w-full justify-start" disabled={sessions.length === 0}>
            <Download className="mr-2 h-4 w-4" />
            <span className="group-data-[collapsible=icon]:hidden">Export All</span>
          </Button>
        <div className="flex items-center justify-between group-data-[collapsible=icon]:flex-col group-data-[collapsible=icon]:gap-2">
          <SettingsDialog apiEndpoint={apiEndpoint} setApiEndpoint={setApiEndpoint} />
          <ThemeToggle />
        </div>
      </SidebarFooter>
    </Sidebar>
